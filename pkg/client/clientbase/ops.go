...
func (a *APIOperations) DoGet(url string, opts *ListOpts, respObject interface{}) error {
	if opts == nil {
		opts = NewListOpts()
	}
...
func (a *APIOperations) DoList(schemaType string, opts *ListOpts, respObject interface{}) error {
	schema, ok := a.Types[schemaType]
	if !ok {
		return errors.New("Unknown schema type [" + schemaType + "]")
	}
	schemaLinks := a.TypeLinks[schemaType]

	if !contains(schema.CollectionMethods, "GET") {
		return errors.New("Resource type [" + schemaType + "] is not listable")
	}

	collectionURL, ok := schemaLinks["collection"]
	if !ok {
...
func (a *APIOperations) DoCreate(schemaType string, createObj interface{}, respObject interface{}) error {
	...
	if !ok {
		...
		collectionURL = re.ReplaceAllString(schemaLinks[SELF], schema.PluralName)
	}
