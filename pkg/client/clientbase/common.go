...
type APIBaseClientInterface interface {
	...
	List(schemaType string, opts *ListOpts, respObject interface{}) error
...
type APIBaseClient struct {
	Ops       *APIOperations
	Opts      *ClientOpts
	Types     map[string]types.APISchema
	TypeLinks map[string]map[string]string
}
...
func NewAPIClient(opts *ClientOpts) (APIBaseClient, error) {
	...
	for _, schema := range schemas.Data {
		result.Types[schema.ID] = *&schema.APISchema
		linkMap := make(map[string]string, len(schema.Links))
		for k, v := range schema.Links {
			linkMap[k] = v
		}
		result.TypeLinks[schema.ID] = linkMap
	}
...
func (a *APIBaseClient) Websocket(url string, _ map[string][]string) (*websocket.Conn, *http.Response, error) {
...
func (a *APIBaseClient) List(schemaType string, opts *ListOpts, respObject interface{}) error {
...
func (a *APIBaseClient) GetLink(resource types.Resource, link string, respObject interface{}) error {
	...
	return a.Ops.DoGet(url, &ListOpts{}, respObject)
}
